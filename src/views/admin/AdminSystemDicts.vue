...
const message = ref('')
const error = ref('')
const overview = ref<SystemDictOverview | null>(null)
const types = ref<SystemDictType[]>([])
const items = ref<SystemDictItem[]>([])
const selectedType = ref<SystemDictType | null>(null)
...
async function loadOverview() {
  const res = normalizeResponse(await getSystemDictOverview())
  if (res.code === 0) {
...
async function loadTypes() {
  ...
  const res = normalizeResponse(await listSystemDictTypes(params))
  if (res.code === 0) {
...
async function loadItems() {
  ...
  const res = normalizeResponse(await listSystemDictItems(params))
  if (res.code === 0) {
...
function selectType(type: SystemDictType) {
...
function editType(type: SystemDictType) {
  typeForm.value = {
    id: type.id,
...
function editItem(item: SystemDictItem) {
  itemForm.value = {
    id: item.id,
...
async function saveType() {
  ...
  try {
    ...
    const res = normalizeResponse(isTypeEdit.value
      ? await updateSystemDictType(typeForm.value.id, payload)
...
async function saveItem() {
  ...
  try {
    ...
    const res = normalizeResponse(isItemEdit.value
      ? await updateSystemDictItem(itemForm.value.id, payload)
...
async function switchType(type: SystemDictType) {
  resetMessage()
  try {
    const res = normalizeResponse(type.status === 1 ? await disableSystemDictType(type.id) : await enableSystemDictType(type.id))
    if (res.code === 0) {
...
async function switchItem(item: SystemDictItem) {
  resetMessage()
  try {
    const res = normalizeResponse(item.status === 1 ? await disableSystemDictItem(item.id) : await enableSystemDictItem(item.id))
    if (res.code === 0) {
...
async function removeType(type: SystemDictType) {
  if (!window.confirm(`确认删除字典类型 ${type.typeCode}？字典类型下有字典项时不能删除。`)) return
  resetMessage()
  try {
    const res = normalizeResponse(await deleteSystemDictType(type.id))
    if (res.code === 0) {
...
async function removeItem(item: SystemDictItem) {
  if (!window.confirm(`确认删除字典项 ${item.itemLabel}？`)) return
  resetMessage()
  try {
    const res = normalizeResponse(await deleteSystemDictItem(item.id))
    if (res.code === 0) {
